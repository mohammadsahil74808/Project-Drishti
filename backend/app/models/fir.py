...
class FIR(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    crime_type: Mapped[CrimeType] = mapped_column(ENUM(CrimeType, name="crime_type", create_type=True), nullable=False, index=True)
    ...
    status: Mapped[CaseStatus] = mapped_column(ENUM(CaseStatus, name="case_status", create_type=True), nullable=False, server_default="open", index=True)
